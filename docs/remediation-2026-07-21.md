...
**Status: fixed and verified** (23 Jul) — recurrence on week/month/year pages built, deployed and exercised on-device; the back-fill regression it surfaced was fixed (d61fca7) and is live. Decisions (23 Jul, Gary): (a) defer collection recurrence — a collection has no natural "next page", and `keyScope` returns null for `col:*` keys so the option simply doesn't appear there; (b) on week/month/year pages the cadence is locked to the page's own scope ("every N months, on each monthly page"), avoiding nonsensical cross-scope combos. Day pages keep the full unit choice unchanged.
...
**Follow-up fix (23 Jul):** making an entry that already existed on a past page recurring was retroactively back-filling open occurrences on the intervening past pages, which then triggered the "review and migrate" banner. `materialisedThrough` is now seeded to the later of the entry's page and the current period, so recurrence never invents past occurrences (honest history); future-dated pages keep their own anchor. This was pre-existing for day-scope recurrence but surfaced while testing item 8. Note: rules created before this fix keep any stray back-filled instances until cleared by hand.
...
**Status: done (27 Jul), pending on-device confirmation.** Both halves now shipped.
**Decision (21 Jul, Gary):** wipe on explicit sign-out only.
**Fix:**
- [x] Explicit sign-out removes the journal and keys from the device, behind a clear warning that requires confirming the journal key is saved (unsynced changes and the key are otherwise unrecoverable; server holds ciphertext only). **Done (27 Jul):** `signOutAndWipe` in `store/sync.ts` tears down the channel, signs out of Supabase, then erases the local Yjs doc (`wipeLocalJournal` → `persistence.clearData()`) and the device keyring (`wipeKeys` → `indexedDB.deleteDatabase("journlet-keys")`), clears the fired-reminders map and resets the active volume, and the Sync screen reloads the app so a fresh empty journal and new keyring generate silently. The Sync "sign out" control is now a two-step confirm: a warning plus a required "I have saved my journal key" checkbox gate the danger button. Kept entirely separate from the involuntary path.
- [x] Session expiry or other involuntary sign-out must NOT wipe; instead show a prominent "not syncing" banner on the journal spread itself, not just the header badge. **Done (2c588a8):** new `ui/NotSyncingBanner`, shown on every non-sync view whenever `syncStatus === "signed-out"` and the device holds local content, gated by a `hasLocalContent` check so a fresh empty install stays quiet. Tappable, routes to the Sync screen; reuses the themed banner styling with the title in danger colour. Does not wipe anything. The involuntary path (`onAuthStateChange` in `startSync`) only calls `teardown()`, never `signOutAndWipe`, so expiry never erases data.
- [x] Make the existing sign-in gate on linking explicit in the Sync screen copy. **Done (27 Jul):** the signed-out block now states that linking to an existing journal happens after sign-in (enter the journal key when asked).
- [x] Update the "Lost a device?" copy if needed — its "no one can remotely erase it" claim stays true; wiping is local and voluntary only. **Reviewed (27 Jul):** claim still accurate, left unchanged.
**Verification:** typecheck clean; full suite green (128 tests) including a new `tests/keystore.test.ts` proving `wipeKeys` erases the ring so the next `ensureKeys` generates fresh key material.
...
**Status: fixed and verified** (23 Jul) — built, deployed and used on-device; header layout iterated with Gary (see below). New full-page `MenuView`, reached by a plainly labelled "menu" button in the header. Acts as the single hub for everything that isn't the current journal page: a "Go to" section with Index at the top, then Sync (status line + "open sync"), Export ("export journal", moved off the Index page), Notifications (permission state + "turn on"), and a marked Preferences placeholder for item 12. Capture footer suppressed on the menu view; export handler and Markdown build moved from `IndexView` to `App`. Typecheck clean.
...
**Status: design approved + schema plumbing done (24 Jul), ritual deferred.** Full design in `docs/volume-schema-design.md` (decisions locked with Gary). Boundary is user-triggered close nudged by size, not calendar year; a volume is an opaque id. The cheap, load-bearing part shipped (45289c1): `journal_updates` gains a `volume` column (backfilled 'v1'), and the client tags pushes, filters the pull, and guards the realtime handler by active volume, with a per-volume IndexedDB doc name (`lib/volume.ts`) that stays `journlet-journal-v1` by default so no local data moves. Done now precisely because all data is one volume, making it a relabel with no re-encryption. Still to build (later, before the active volume grows heavy): the close-volume ritual (carry open tasks/future entries/rules forward, `closed` marker, archive read-only), the size nudge, the permanent `shared` collections/habits store, per-volume reconcile cursor, and volume-switch UI. **Note: the SQL migration in `supabase/schema.sql` must be run on the live Supabase project.**

...
**Feedback.** *Menu → Send feedback* composes a report addressed to hello@journlet.com and offers three plainly labelled ways to send it: Gmail's own composer in a new tab, a `mailto:` for whatever mail app the device is set up with, and the clipboard, which is the route for any other webmail. The copied text is not identical to what the links send: it carries the subject as its first line, because a pasted message has no subject field and the one chosen on the screen was otherwise dropped (a report pasted into iCloud Mail arrived as "(no subject)"). Nothing is detected, because nothing can be: a page cannot ask whether a `mailto:` handler exists, and the first version of this screen made that guess and met somebody with no mail client, for whom the primary action opened macOS Mail's Add Account dialog. The two link routes carry different length limits, 1,800 characters for the mail link and 7,000 for the browser one, and a route over its limit is withdrawn rather than left to truncate. The diagnostics block is shown in an editable box before it goes anywhere and carries no journal content: build, commit, installed or tab, user agent, window size, network, sync state, the last server error, and a count and size for the journal. The draft is kept in local storage on that device so being offline or closing the app does not lose it, and it is never added to the journal or sent to the server. The screen claims only that a mail app was opened, because that is all it can know. A report too long for a mail link is refused rather than truncated, with the clipboard offered instead. Deliberately not a Supabase table: see spec §13.1 for the three reasons, and §11 Q18 for the aggregate signal this route does not provide.
...
5. Project Settings → API: copy the project URL and anon key into `src/lib/supabaseConfig.ts`, then commit and push. That is the only file to change: the `connect-src` directive of the CSP meta tag in `index.html` carries a `%SUPABASE_ORIGINS%` placeholder, and the build substitutes `https://` and `wss://` for the host in `SUPABASE_URL` (see `build/csp.ts`). It used to be both files, and changing one without the other broke sync silently, because the CSP blocks the request before Supabase sees it so there is no server error to report and the not-syncing banner cannot say why. Writing a host back into the markup now fails the build. Both values are safe to publish, RLS and E2EE do the guarding.
...
- `package.json` overrides `@trickfilm400/rollup-plugin-off-main-thread`, a personal-namespace republication of Google's archived `@surma/rollup-plugin-off-main-thread` that `workbox-build@7.4.1` depends on as a `3.0.0-pre1` prerelease. The override points that path back at `@surma/rollup-plugin-off-main-thread@2.2.3`. It matters because the package is evaluated on every build: we use `injectManifest` and never call its transform, but `workbox-build`'s entry eagerly requires `./generate-sw`, which requires `lib/bundle.js`, which requires this. Its job is bundling the service worker, which is the one piece of code that persists across navigations, intercepts every request and runs with no CSP at all on Pages, so it is the worst position in the tree to have a surprise in. This narrows the finding rather than closing it: the replacement is also a single-maintainer namespace, and it would break `generateSW`, which this project does not use. The durable fix is dropping `vite-plugin-pwa` and bundling the worker directly.
